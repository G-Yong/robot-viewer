...
- **交互式 IK** —— 拖动末端手柄即可摆位：箭头平移、圆环绕基座坐标轴旋转，两者**同时显示**（在视口里按 `W` / `E` / `Q` 即可筛选，无需去点下拉框）。IK 选项卡里选择 TCP 链节和工具偏置；求解器采用阻尼最小二乘，跟着手柄走，对机械臂无法保持的位姿会拒绝。多臂模型里每个末端只在自己的运动链上求解，拖一个不会带动另一个。
- **末端位姿显示** —— IK 选项卡同时实时显示 TCP 位姿，表示方式与参考坐标系均可自选。位置以毫米显示；姿态可选 **RPY**（固定轴 XYZ，即 URDF/RViz 约定）、**内旋欧拉角** `ZYZ` / `ZXZ` / `XYZ` / `ZYX`（动轴用撇号标注，如 `Z` / `Y′` / `Z″`），或**四元数**（已单位化并固定符号，拖拽时不会跳变）。计算原点可选机器人基座（正是手柄箭头所在的参考系）、视图的世界原点，或模型中的任意链节——“腕心相对法兰”只是下拉框的一下。遇到退化情形会明确提示，而不是悄悄丢掉一个角度。
- **渲染设置** —— 背景颜色、地面网格、环境光/主光强度、视觉/碰撞几何体切换、线框模式、上方向轴（URDF 约定的 `+Z` 或 `+Y`）、**着色模式**（*原始* 或 *交替*），以及**坐标轴**：在世界原点与每个关节处显示带颜色的箭头坐标系（可开关，并有坐标轴大小滑块），外加一个**角落视图指南针**，会跟随摄像机并可点击切换到正对某个轴的方向。
- **场景保存/加载** —— 将关节数值、摄像机位姿和渲染设置保存为 `*.robotscene.json` 文件，之后可再次加载还原。
- **可收起的选项卡面板** —— 停靠在侧边的面板，用选项卡（关节、摄像机、IK、渲染、场景、OPC UA）切换功能；可通过边缘的拉手收起/拉出。
