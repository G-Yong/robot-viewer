...
- **渲染设置** —— 背景颜色、地面网格、环境光/主光强度、视觉/碰撞几何体切换、线框模式，以及上方向轴（URDF 约定的 `+Z` 或 `+Y`）。
...
4. **变量命名（Variable Naming）** —— 标识符模板（`{joint}` 会被替换为关节名），以及“应用到所有关节”按钮。
5. **关节映射（Joint Mapping）** —— 每个关节一行的表格：启用/禁用、编辑 NodeId 标识符，并可设置对每个输入值应用的 `scale`（缩放）/ `offset`（偏移）。
