...
- **Rendering settings** — Background color, ground grid, ambient/key light intensity, visual/collision geometry toggles, wireframe, up-axis (`+Z` URDF convention or `+Y`), and a **color mode**: *Original* (use the URDF's own material colors) or *Alternate* (a distinct vivid color per link for easy identification).
